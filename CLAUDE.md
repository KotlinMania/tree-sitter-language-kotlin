# Claude Code Project Instructions — tree-sitter-language-kotlin

## Project Overview

This is **tree-sitter-language-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`tree-sitter-language`](https://crates.io/crates/tree-sitter-language).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/tree-sitter-language/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/tree-sitter-language>

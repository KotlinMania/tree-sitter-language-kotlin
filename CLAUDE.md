# Claude Code Project Instructions — num-bigint-kotlin

## Project Overview

This is **num-bigint-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`num-bigint`](https://crates.io/crates/num-bigint).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/num-bigint/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/num-bigint>

...
kotlin {
    ...
    val xcf = XCFramework("TreeSitterLanguage")
    ...
    swiftExport {
        moduleName = "TreeSitterLanguage"
        flattenPackage = "io.github.kotlinmania.treesitterlanguage"
    }

    android {
        namespace = "io.github.kotlinmania.treesitterlanguage"
        compileSdk = 34
        minSdk = 24
...
mavenPublishing {
    ...
    pom {
        ...
        scm {
            url.set("https://github.com/KotlinMania/tree-sitter-language-kotlin")
            connection.set("scm:git:git://github.com/KotlinMania/tree-sitter-language-kotlin.git")
            developerConnection.set("scm:git:ssh://github.com/KotlinMania/tree-sitter-language-kotlin.git")
        }
    }
}
...
// Explicit audit contract: the default `build` task must compile/link every
// configured Kotlin target, including non-host native test binaries and the
// XCFramework assemblies. Without this wiring, `./gradlew build` reports
// success while silently skipping Apple device-target test links, Android
// Native binaries, linuxArm64, and the XCFramework outputs — the false
// positive the 2026-05-19 async-channel-kotlin incident documents.
val fullTargetBuildTaskNames = setOf(
    ...
    "assembleTreeSitterLanguageXCFramework",
)
...
// CodeQL Java/Kotlin extraction: dedicated kotlinc invocation that compiles
// commonMain through K2JVMCompiler.doExecute so the CodeQL Java agent's
// Kotlin extractor sees the compile. .github/workflows/codeql.yml invokes
// `./gradlew codeqlCompileJvm` to feed kotlinc-compiled commonMain through
...
val codeqlCompileJvm = tasks.register<JavaExec>("codeqlCompileJvm") {
    ...
    doFirst {
        ...
        if (sourceFiles.isEmpty()) {
            val sentinelFile = sentinelDir.get().asFile.resolve("io/github/kotlinmania/treesitterlanguage/codeql/_CodeqlEmptySource.kt")
            ...
            sentinelFile.writeText(
                ...
                package io.github.kotlinmania.treesitterlanguage.codeql

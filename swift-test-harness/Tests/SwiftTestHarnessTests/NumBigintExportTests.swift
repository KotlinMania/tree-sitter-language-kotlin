...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `NumBigint.swiftmodule/`
...
//      `import NumBigint` above would fail at compile time.
//
//   2. The static archive `libNumBigint.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "NumBigint" }` and
//      `flattenPackage = "io.github.kotlinmania.numbigint"` configuration in
...
//      via the `NumBigintLibrary` product.
//
...
final class NumBigintExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "NumBigint swift module imported cleanly")
    }
}
